...
class PointCloudOptimizer(BasePCOptimizer):
    ...
    def __init__(self, *args, optimize_pp=False, focal_break=20, bin_masks=None, **kwargs):
    ...
    def forward(self):
        ...
        if isinstance(self.bin_masks, torch.Tensor):
            e_masks_i, e_masks_j = generate_bg_mask(self.edges, self.bin_masks)
            e_masks_i, e_masks_j = e_masks_i.to(self.device), e_masks_j.to(self.device)
            self._weight_i = self._weight_i * e_masks_i
            self._weight_k = self._weight_j * e_masks_j
        # compute the less
        li = self.dist(proj_pts3d[self._ei], aligned_pred_i, weight=self._weight_i).sum() / self.total_area_i
        lj = self.dist(proj_pts3d[self._ej], aligned_pred_j, weight=self._weight_j).sum() / self.total_area_j
        #print(self._weight_j.shape)
        return li + lj

def generate_bg_mask(edges, mask_raw):
    e_masks_i, e_masks_j = [], []
    for i, j in edges:
        e_masks_i.append(mask_raw[i].reshape((-1,)))
        e_masks_j.append(mask_raw[j].reshape((-1,)))
    e_masks_i, e_masks_j = np.array(e_masks_i), np.array(e_masks_j)

    return torch.tensor(e_masks_i).float(), torch.tensor(e_masks_j).float()
